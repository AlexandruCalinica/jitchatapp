...
export default function ToolbarPlugin(): React.ReactNode {
  ...
  const updateToolbar = React.useCallback(() => {
    ...
    // Check for draft mode in selected nodes
    const nodes = selection.getNodes();
    const hasDraft = nodes.some((node) => {
      if (node.getType() === "extended-text") {
        const extendedNode = node as any;
        return extendedNode.isDraft;
      }
      return false;
    });
    setIsDraft(hasDraft);
  ...
  return (
    <div className="flex items-center">
      ...
      <IconButton
        ...
        icon={<Icon name="edit-one" className="text-inherit" />}
        className={clsx("rounded-sm", {
          [activeStyle]: isDraft,
        })}
        onMouseDown={(e) => {
